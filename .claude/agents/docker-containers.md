...
Both application contexts have `.dockerignore` policies covering local environment
variants, keys/certificates, database files, logs, caches, dependencies, and generated
build/test artifacts recursively. Runtime `data/` is excluded at the context root
only; preserve nested source modules such as `src/data`. Only the root `.env.example` is retained from
the environment file family. Keep source, Go embedded migrations, frontend public
assets, and build configuration available to `COPY`.

Run `npm run test:docker-context` from the repository root after changing an
ignore policy or build input layout. It requires Docker Buildx and a running
builder. The test reads only the two ignore policies from the workspace, creates
synthetic source and excluded-file fixtures, then exports a `FROM scratch` /
`COPY . /context/` build to verify Docker's matcher. It neither reads actual local
environment files nor needs to pull a base image. `Build Docker Images` runs this
probe before application builds.

Compose supplies API secrets through runtime `environment` configuration. Local
frontend env files are excluded from image inputs; the current frontend
Dockerfile uses source defaults, with no runtime nginx environment injection.
Treat any deliberate Vite build configuration as public browser-visible input.
...
- Keep the Playwright Dockerfile image aligned with the installed
  ...
  Run `npm run sync:image-pins` from the repo root to fix drift in either pin:
  it resolves the `.nvmrc`-driven `node` tag and the lockfile-driven Playwright
  tag to their current registry digests and rewrites `ARG PLAYWRIGHT_IMAGE`
  plus the `node` pins in all three frontend Dockerfiles. It needs network
  access and is intentionally not a CI step, so the offline policy test remains
  the gate.
