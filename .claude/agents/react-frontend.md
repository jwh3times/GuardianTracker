...
frontend/src/
  ...
  data/                         ← ADR 0020 data-access modules, one per domain resource. A module
    ...
    membershipRefresh.ts         ← `useMembershipRefresh()` owns the cache-refresh endpoint
                                   ...
                                   membership at all. Remaining resources (Catalysts, Crafting, Seals)
                                   have not migrated yet — see the ADR 0020 note under "Shared query
                                   definitions" below.
  styles/
...
Wish list, Collections, Weekly, Characters, Items, Search, Flags, and Admin no
longer follow this raw pattern — they are owned by `src/data/` modules (see
below); their queries and mutations are not declared inline in a feature.
...
Search (E9), Flags (E10), and Admin (E11) have landed: `data/wishlist.ts` owns the wish list
...
they both did before these modules existed. `data/membershipRefresh.ts` holds the
...
declaring their own. The remaining resources — Catalysts, Crafting, and
