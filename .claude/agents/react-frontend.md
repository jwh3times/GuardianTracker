...
frontend/src/
  ...
  contexts/
    AppProviders.tsx             ← The single definition of the context tower. `AppProviders`
                                   (keyed IdentityContext > QueryClientProvider > AuthProvider > PreferencesProvider >
                                   ...
                                   replace the client and remount this subtree.
  ...
  data/                         ← ADR 0020 data-access modules, one per domain resource. A module
    ...
    catalysts.ts                  ← Ninth resource landed (E12). Private query key
                                   ...
                                   entry point. Read by the Catalysts & Crafting page; that page's
                                   crafting query remains inline until E13.
    membershipRefresh.ts         ← `useMembershipRefresh()` owns the cache-refresh endpoint
                                   ...
                                   `invalidateWeekly`, `invalidateCharacters`, and `invalidateCatalysts`
                                   for the four migrated resources; the remaining two (crafting, seals)
                                   are raw keys in one `UNMIGRATED_KEYS` list, each replaced by that
                                   resource's own invalidation entry point as its ADR 0020 slice lands.
                                   ...
                                   membership at all. Remaining resources (Crafting, Seals) have not
                                   migrated yet — see the ADR 0020 note under "Shared query definitions"
                                   below.
  styles/
  ...
  features/                    ← Feature slices; each owns its pages, feature-only components, helpers,
    ...
    collections/               ← Collections.tsx (tree + grid/list + detail drawer via data/collections.ts's useCollections;
                                   ...
                                   only renders them),
                                   Catalysts.tsx (catalysts via `data/catalysts.ts`'s useCatalysts; crafting
                                   still an inline useQuery until E13; cards render `effect` text when
...
Wish list, Collections, Weekly, Characters, Items, Search, Flags, Admin, and
Catalysts no longer follow this raw pattern — they are owned by `src/data/`
modules (see below); their queries and mutations are not declared inline in a
feature.
...
Search (E9), Flags (E10), Admin (E11), and Catalysts (E12) have landed:
...
declaring their own. The remaining resources — Crafting and Seals — are
sequenced by the #172 handoff and have not migrated yet, so for those, the
...
type/ID cancels and clears the old QueryClient, creates a replacement, and
remounts the subtree, resetting preferences, onboarding, flags, character state,
and page drafts. Same-membership refresh retains caches and mounted state.
`useIdentityMutation` guards delayed mutation starts and completion callbacks;
late cache work remains isolated in the retired client. Use the current client
from `useQueryClient()` for cache operations rather than importing the initial
client singleton.
...
`PreferencesContext` (`contexts/PreferencesContext.tsx`) holds card style, the
"for you" badge preference, and Guardian Tracker user-backed onboarding
completion, mirroring card style and personalization into the global
`guardian_prefs` localStorage key. Composition resets this snapshot at identity
boundaries; the new provider starts with defaults, and identity guards discard
departed reads, onboarding results, and setter calls.

[ADR 0021](../../docs/adr/0021-own-preferences-synchronization.md) accepts
replacing it with a framework-neutral preferences client at
