...
- Browser suites live under `frontend/e2e/` and use Playwright 1.62.0 plus `@axe-core/playwright`. Run `npm run e2e` for functional + accessibility + destructive-auth projects and `npm run e2e:visual` for visual comparison.
...
- CI has exactly one retry for functional/axe, traces on the first retry, and retains reports/screenshots/video/diffs for 14 days. Visual baselines use 1440x900 Chromium in `mcr.microsoft.com/playwright:v1.62.0-noble` and stay advisory.
