...
- CI has exactly one retry for functional/axe, traces on the first retry, and retains reports/screenshots/video/diffs for 14 days. Visual baselines use 1440x900 Chromium in a `mcr.microsoft.com/playwright` image whose tag CI derives from `frontend/package-lock.json`, and stay advisory. Never hardcode that tag — it must track `@playwright/test`.
