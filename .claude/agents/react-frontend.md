...
frontend/src/
  ...
  lib/
    ...
    queries.ts                 ← Items-only until E8. itemPerksQuery(itemHash) — lazy perk-pool fetch
                                   (staleTime: Infinity); itemByHashQuery(itemHash) — minimal item view
                                   for deep-link miss resolution (staleTime: Infinity, retry: false)
...
`lib/queries.ts` is now Items-only. Collections' counts-only and full queries
moved out of here into `data/collections.ts` as `useCollectionsSummary()` /
`useCollections()` (ADR 0020, E5), and the This Week payload moved out into
`data/weekly.ts` as `useWeekly()` (ADR 0020, E6); see the `data/` section
above for both.
