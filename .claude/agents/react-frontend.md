...
frontend/src/
  ...
  data/                         ← ADR 0020 data-access modules, one per domain resource. A module
    ...
    membershipRefresh.ts         ← `useMembershipRefresh()` owns the cache-refresh endpoint
                                   ...
                                   has no claim on the other five. Calls `invalidateCollections` for
                                   the one migrated resource; the other five (characters, weekly,
                                   catalysts, crafting, seals) are raw keys in one `UNMIGRATED_KEYS`
                                   list, each replaced by that resource's own invalidation entry point
                                   as its ADR 0020 slice lands. Read no membership argument — it reads
                                   the signed-in membership from `useAuth()`.
                                   Remaining resources (Weekly, Characters, Items, Search, Flags,
  ...
  lib/
    ...
    queries.ts                 ← itemPerksQuery(itemHash) — lazy perk-pool fetch (staleTime: Infinity);
...
Collections' counts-only and full queries moved out of here into
`data/collections.ts` as `useCollectionsSummary()` / `useCollections()` (ADR
0020, E5); see the `data/` section above.
...
Wish list (E4) and Collections (E5) have landed: `data/wishlist.ts` owns the
wish list resource's query identity, projection, and all six mutations, and
`data/collections.ts` owns the collections resource's query identity and
projection dispatch, plus `data/membershipRefresh.ts` for the cross-resource
cache-refresh fan-out. Consumers read these modules instead of declaring their
own. The remaining resources — Weekly, Characters, Items, Search, Flags,
Admin, Catalysts, Crafting, Seals — are sequenced by the #172 handoff and have
not migrated yet, so for those, `lib/queries.ts`, the per-feature `useQuery` /
