...
frontend/src/
  ...
  lib/
    ...
    queries.ts                 ← collectionsQuery(membershipType, membershipId) — counts-only collections
                                   query (select: toCollectionsSummary), used by Dashboard, Settings, and
                                   OnboardingTour; collectionsFullQuery(membershipType, membershipId) — full
                                   collections query with items (select: toCollections), used by Collections
                                   and Cosmetics. Two functions rather than a boolean flag because the variants
                                   return different view types (see lib/collectionsView.ts); cache keys are
...
- `collectionsQuery(membershipType, membershipId)` — the counts-only collections query (`select: toCollectionsSummary` from `lib/collectionsView.ts`); Dashboard, Settings, and OnboardingTour all use it so they share a single cache entry per membership instead of firing separate requests.
