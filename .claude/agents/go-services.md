...
- `DestinyItem.AcquisitionSources` is the deterministic union from all collectibles
  linked to the item. `DestinyItem` has no item-level `Difficulty` or raw `Sources`.
- `DestinyItem.FarmOnly` retains the representative collectible's existing
  "cannot be reacquired" check. Multi-collectible farm-only semantics remain
  unresolved; do not derive them from the acquisition-source union without a new
  decision.
