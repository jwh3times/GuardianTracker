...
backend/api-service/
  ...
  services/rolltargets/rolltargets.go  ← Domain vocabulary: TargetID, StoredTarget (ItemHash *uint32,
                                           ...
                                           PlugNames, WeaponPerkNames). Perks are stored as display names,
                                           not plug hashes —
  ...
  services/rolltargets/dimimport.go    ← Service.ImportDIM(ctx, membershipID, text) (ImportReport, error).
                                           ...
                                           PerkPool.PlugNames instead, since it has no weapon pool — but that
                                           only resolves a hash to whatever name its plug carries, which may
                                           be a mod or other non-perk plug; Add's validateAnyWeaponPerks check
                                           against PerkPool.WeaponPerkNames catches that case and reports it
                                           as unresolved perk rather than importing.
...
| GET    | `/api/rolltargets`                                                            | JWT                           | List the caller's roll targets; `[]` when none. Membership read solely from the JWT — no route param, no query string                                                                                                                                                                                                                                                                                                                                      |
| POST   | `/api/rolltargets`                                                            | JWT                           | Add a roll target; `{itemHash?, wanted?, perks, notes?}` — omitted `itemHash` saves an any-weapon target, omitted `wanted` defaults to `true`; 201 on success                                                                                                                                                                                                                                                                                              |
