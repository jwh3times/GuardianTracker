...
| `db/rolltargets.go`          | `RollTargetStore` — roll target CRUD, user-scoped (`GetUserID`, `List`, `Add`, `Update`, `Delete`); `perks` persists as a `TEXT[]` of display names, not plug hashes. Consumed only through `db/adapters/rolltargets.go`. Backs `services/rolltargets`, which has no HTTP route or `main.go` wiring yet                                                                                                        |
...
| `db/adapters/rolltargets.go` | Membership-keyed adapter for `services/rolltargets`; resolves the internal user ID and translates PostgreSQL's failure vocabulary — duplicate → `ErrDuplicate`, `pgx.ErrNoRows`/not-found → `ErrNotFound`, `db.ErrUnavailable` → `rolltargets.ErrUnavailable`                                                                                                                                                  |
...
-- Roll targets (migration 0009): the player's own saved perk combination
-- wanted on a weapon. perks holds display names, not plug hashes (a perk's
-- base/enhanced variants share one name but no manifest field links the two
-- hashes). UNIQUE (user_id, item_hash) — one target per weapon per user.
-- No HTTP route or main.go wiring yet.
roll_targets (id, user_id FK, item_hash, perks TEXT[], notes,
