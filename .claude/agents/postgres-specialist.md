...
| `db/degraded.go`             | `ErrUnavailable` sentinel + the six store interfaces + degraded implementations whose every method returns `ErrUnavailable`. Admin/audit/account handlers, which call a store directly, map it to a 503 via `handlers.HandleStoreError`; wishlist and preferences instead go through their own domain adapter and typed handler error mapping (see the two `db/adapters/*.go` rows below)         |
...
docker run --name guardian-pg `
  ...
  -d postgres:18.6-alpine3.24@sha256:d3e1620b530c944afa6e887d22eb899824da68e19c52024bf98f5220c88a65b2
