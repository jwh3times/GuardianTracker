...
| `db/stores.go`               | `Stores` struct — interface fields (`UserRepo`, `TokenRepo`, `WishlistRepo`, `PrefsRepo`, `DigestRepo`, `FlagRepo`, `AuditRepo`, `Pinger`), never nil; `NewStores(nil)` returns the degraded set; `Available() bool` reports whether persistence is real                                                                                                                                          |
| `db/degraded.go`             | `ErrUnavailable` sentinel + the seven store interfaces + degraded implementations whose every method returns `ErrUnavailable`. Admin/audit/account handlers, which call a store directly, map it to a 503 via `handlers.HandleStoreError`; wishlist, preferences, and digest instead go through their own domain adapter and typed handler error mapping (see the `db/adapters/*.go` rows below)  |
