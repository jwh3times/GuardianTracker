...
| `scripts/sync-agents.mjs`                     | Maintainers                        | Generates `.codex/agents/` from `.claude/agents/` and `.claude/skills/` from `.agents/skills/`; edit when the sync contract changes |
...
public agent guide. Private issue and wiki handoffs required by the human-follow-up
procedure below are authorized as part of completing the task. Other private
content updates require an explicit request or relevance to the active change.
...
- `AGENTS.md`: its `## CI/CD` section — job names, coverage gates, action and Go
  tool pinning, Staticcheck version, required status checks, versioning/changelog behavior
...
- Do not edit `.codex/agents/*.toml` or `.claude/skills/**`. They are generated —
  ...
  by `scripts/sync-agents.mjs`. Edit the authored source instead (`.claude/agents/`
  for agents, `.agents/skills/` for skills); the script regenerates the mirrors and CI
  fails if they drift.

## Human follow-ups before handoff

Before the final documentation handoff, account for every human action arising
from the completed agent work. Follow
[Human follow-ups](../../docs/agents/human-followups.md), the canonical procedure:
search for and reuse an existing follow-up where possible, then ensure each
action has a private companion issue, a project-board entry marked
`ready-for-human`, and private wiki instructions with step-by-step execution and
verification. Link the issue and wiki instructions in both directions. Keep live
status on the issue and board.

Report the resulting handoff links privately. If access prevents creating or
updating an issue, board entry, or wiki instructions, identify the missing
artifact and access limitation honestly; do not report the handoff complete.
...
- human follow-ups recorded through the canonical procedure, or the exact artifacts still blocked by access
