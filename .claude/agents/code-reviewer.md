...
- Flag endpoints that return another user's data without verifying the requesting user's full Destiny membership pair (`membershipType` + `membershipId`) from the JWT (`ownershipCheck` in `api/handlers/common.go` checks both, not `membershipId` alone).
...
- The visual job builds `frontend/Dockerfile.playwright`, whose `ARG PLAYWRIGHT_IMAGE` pins the tag and digest literally; `scripts/node-version-policy.test.mjs` requires that tag to equal the `@playwright/test` version in `frontend/package-lock.json`. No ecosystem bumps it — Dependabot's npm update never edits a Dockerfile — so a Playwright bump must carry a matching ARG change, produced by `npm run sync:image-pins` (which also covers the `.nvmrc`-driven `node` pins in all three frontend Dockerfiles). Treat a bump whose ARG was not updated as the defect, not the updated pin. Keep `workers: 1`, exactly one functional CI retry, and destructive auth sequenced after shared journeys.
...
- `PUT /api/account/role` — self-service role opt-in; rejects `admin` as a target and checks the locked current DB role through `SetSelfRole`, returning `ADMIN_OPT_IN` for admin callers even with stale middleware state. Preserve atomic role-update/`role.optin` audit commit, DB-derived `oldRole`, and session/token-version preservation. Successful bootstrap admin upserts evict the local authorization cache; cross-replica cache lag remains bounded by the normal TTL.
