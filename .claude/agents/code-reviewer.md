...
- The state parameter in `POST /api/auth/bungie/callback` is verified via `auth.StateSigner.Verify()`. This is not single-use by design (stateless HMAC). Do not add a single-use map — see the known limitations in AGENTS.md.
