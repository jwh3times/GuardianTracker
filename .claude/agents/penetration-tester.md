...
The path `membershipType`/`membershipId` pair must be validated together against the JWT's `membership_type`/`membership_id` claims — `ownershipCheck` (`api/handlers/common.go`) checks both, not `membershipId` alone. The same helper guards both character routes and the catalysts/crafting/seals routes.
