...
**Manifest publication** (`manifeststate.Publication`) — reusable state derived
from one installed Manifest generation. It belongs to one Manifest observer;
work begun before that observer advances may finish for its initiating request,
...
worth chasing. An unmatched target may carry its **best copy**: the applicable
