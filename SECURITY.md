...
- **Browser lifecycle coordination** — one shared browser session client owns callback completion, authenticated transport, refresh, and local session termination. Web Locks serialize shared-cookie lifecycle operations across same-origin tabs; sign-in completion and refresh fail when coordination is unavailable. Logout persists an anonymous envelope before best-effort server cleanup, and remains locally available without Web Locks. `AuthProvider` only projects the client's user/authenticated snapshot and performs no hydration profile request or JWT decoding.
