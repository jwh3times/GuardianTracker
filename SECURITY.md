...
- **Browser identity isolation** — logout and Destiny membership changes cancel and clear the departing query cache, replace its QueryClient, remount providers and page state, and run registered identity-boundary resets, including the preferences client. Old mutation starts and completion callbacks are guarded by their originating identity; the preferences client fences its own asynchronous completions by an internal epoch and discards departed results. Same-membership refresh preserves caches and mounted state and runs no boundary resets. These browser controls complement server-side authorization.
- **Owner-only character surfaces** — the equipment, recent-activity, and
  current-activity routes validate the caller's complete Destiny membership
  pair before reading the Bungie authorization or calling Bungie, and reject
  characters outside that membership. Bungie item-instance identifiers are used
  only for server-side equipment stat correlation and are not exposed; activity
  and current-activity responses omit Bungie instance identifiers, and current
  activity carries no membership or character identifiers either.
...
- **Admin bootstrap** — admin can only be minted via `ADMIN_MEMBERSHIP_IDS` (comma-separated Bungie membership IDs pinned to admin on every login upsert) or granted by an existing admin. There is **no self-service path to admin**: `PUT /api/account/role` accepts only standard/beta/alpha and rejects admin callers. The store locks and checks the current database row before updating, so a stale request role cannot override a concurrent admin promotion. Rejection returns `403 ADMIN_OPT_IN`. A successful bootstrap upsert evicts the local authorization cache; other replicas retain the normal 60-second cache window. `ADMIN_MEMBERSHIP_IDS` is non-secret config (membership IDs), but treat it as sensitive — it controls who holds admin.
...
- **Audit trail** — authentication events (login success/failure, logout, logout-all, refresh success/failure), session security events (refresh-token reuse, session termination), self opt-in role changes, admin role changes, and feature-flag changes are persisted to the unified `audit_log` table and exposed to admins via `GET /api/admin/audit` and the `/admin` Audit Log UI panel. Self-service opt-in commits the role update and `role.optin` event together, with `oldRole` read from the locked database row; an audit write failure rolls back the update. Admin role and flag changes also write their audit events in the mutation transaction; auth/session events are best-effort (a DB outage can drop an event). Successful refresh emits `refresh.success` with membership and session identifiers from the verified issuance result, plus request IP/User-Agent; it records no access token, refresh token, or cookie value. Audit persistence failure does not change an otherwise successful refresh response or its replacement cookie. **Client IP address and User-Agent are captured and stored** for security forensics, retained for `AUDIT_RETENTION_DAYS` (default 180), and pruned hourly once past expiry. IP addresses are trusted only from `TRUSTED_PROXIES` (gin `SetTrustedProxies`) so they cannot be spoofed by a client; configure this in production to your platform's ingress range.
...
- **Frontend response headers**: nginx merges server security headers into the static-asset location, preserving them alongside immutable asset caching and on asset errors. `npm run test:nginx-headers` verifies real HTTP responses from the pinned runtime image in `Build Docker Images`.
