...
- **Owner-only character surfaces** — the equipment and recent-activity routes
  validate the caller's complete Destiny membership pair before reading the
  Bungie authorization or calling Bungie, and reject characters outside that
  membership. Bungie item-instance identifiers are used only for server-side
  equipment stat correlation and are not exposed; activity instance identifiers
  are not included in the recent-history projection.
