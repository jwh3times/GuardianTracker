...
- **Owner-only character equipment** — the equipment route validates the caller's
  complete Destiny membership pair before reading the Bungie authorization or
  calling Bungie, and rejects characters outside that membership. Bungie item
  instance identifiers are used only for server-side stat correlation and are not
